...
def test_condemn_working_equipment(equipment, engineer):
    condemn_equipment(
        equipment, engineer, remark="beyond repair", condemned_location="Store Room B"
    )
...
def test_condemn_cascades_to_workorder_and_complaints(equipment, staff_user, engineer):
    ...
    condemn_equipment(
        equipment, engineer, remark="unsafe", condemned_location="Disposal yard"
    )
    equipment.refresh_from_db()
    wo.refresh_from_db()
    ...
    assert wo.remarks.filter(
        kind=RemarkKind.SYSTEM, text__icontains="condemned"
    ).exists()
...
def test_cannot_condemn_twice(equipment, engineer):
    ...
    with pytest.raises(InvalidTransition):
        condemn_equipment(
            equipment, engineer, remark="again", condemned_location="Store"
        )
